...
function DealCard({
  ...
  return (
    ...
    >
      <Card className="group flex h-full flex-col overflow-hidden border-white/15 bg-white/80 shadow-glass transition hover:-translate-y-1 hover:border-fitidion-orange/40 hover:shadow-fitidion dark:border-white/10 dark:bg-slate-900/60">
        <CardHeader className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
            ...
            {countdown && (
              <span className="inline-flex items-center gap-1 rounded-full bg-fitidion-orange/10 px-3 py-1 text-fitidion-orange">
                ⏱️ {countdown}
              </span>
            )}
          </div>
          <div>
            <h3 className="text-xl font-semibold text-fitidion-dark dark:text-white">{deal.title}</h3>
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{deal.vendor}</p>
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-fitidion-orange/80">{deal.source}</p>
          </div>
        </CardHeader>
        <CardContent className="flex flex-1 flex-col justify-between space-y-6">
          ...
          >
            {ratingValue ? (
                  ...
                  {Array.from({ length: 5 }).map((_, starIndex) => {
                    ...
                    return (
                      <Star
                        ...
                        className={`h-4 w-4 ${isFilled ? "fill-fitidion-orange text-fitidion-orange" : "text-white/30 dark:text-slate-700"}`}
                        aria-hidden="true"
                      />
                    );
                  })}
                </div>
                <span className="font-semibold text-fitidion-dark dark:text-white">
              ...
              <span className="text-xs text-slate-400 dark:text-slate-500">Avis à venir</span>
            )}
          </div>
          <div>
            <div className="flex items-end gap-2">
              <p className="text-3xl font-bold text-fitidion-dark dark:text-white">{formattedPrice}</p>
              {referencePrice && <span className="text-sm text-slate-400 dark:text-slate-500 line-through">{referencePrice}</span>}
            </div>
            {deal.shippingText && <p className="text-xs text-slate-400 dark:text-slate-500">{deal.shippingText}</p>}
          </div>
          {deal.link ? (
  ...
  return (
    <section id="promotions" className="relative overflow-hidden py-24">
      ...
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6">
        ...
        {usingFallback && (
          <p className="mt-2 text-sm font-medium text-fitidion-orange">
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
          {state !== "success"
            ...
            : (
                <p className="col-span-full rounded-3xl border border-white/20 bg-white/10 p-8 text-center text-slate-600 dark:text-slate-300">
